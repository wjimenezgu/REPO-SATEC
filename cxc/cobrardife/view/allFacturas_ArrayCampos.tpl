					...
					<input type="hidden" name="idTienda" id="idTienda" value="{$ID_TIENDA}">
						<fieldset>
										...
										<tbody id="tbod">
                                			<tr>
                                				<td colspan="2">
													...
													<div class="controls">
								  						<select onChange="revSeleccion(1)" disabled id="selectTipoPago[]" style="margin-left: -95px; width:110px; ">																												
												...
												<div class="controls">
								  					<input  style="margin-left: -95px; width:80px; " id="idAut[]" disabled name="idAut[]" onblur="reviAuto(1);" type="text" value="">
												</div>
                                			
                                				</td>
                                				<td>Monto</td>
                                				<td style="text-align:right;">
                                					<input type="text" id="fpagaCon[]" name="fpagaCon[]" value="0.00"  disabled style="color: green; font-size: 130%; font-weight: bold; text-align: right;height:40px; width: 110px;" >
                                					<input type="hidden" id="fsaldoNota[]" name="fsaldoNota[]" value="0.00" >
                                				</td>
                                				<td>
										...
										<tbody id="tbod">
                                			<tr>
                                				<td colspan="2">
													<label style="float: left; width:70; padding-top: 5px; text-align: right;" for="selectTipoPago">T. Pago</label>
													<div class="controls">
								  						<select onChange="revSeleccion(2)" disabled id="selectTipoPago[]" style="margin-left: -95px; width:110px; ">
                                        					{section name="p" loop=$LISTA_FPAGOS}
                                            					<option value="{$LISTA_FPAGOS[p].id}">{$LISTA_FPAGOS[p].descCorta}</option>
                                        					{/section}	
								  						</select>
												...
												<div class="controls">
								  					<input  style="margin-left: -95px; width:80px; " id="idAut[]" disabled name="idAut[]" onblur="reviAuto(2);" type="text" value="">
												</div>
										...
										<tbody id="tbod">
                                			<tr>
                                				<td colspan="2">
													<label style="float: left; width:70; padding-top: 5px; text-align: right;" for="selectTipoPago">T. Pago</label>
													<div class="controls">
								  						<select onChange="revSeleccion(3)" disabled id="selectTipoPago[]" style="margin-left: -95px; width:110px; ">
                                        					{section name="p" loop=$LISTA_FPAGOS}
                                            					<option value="{$LISTA_FPAGOS[p].id}">{$LISTA_FPAGOS[p].descCorta}</option>
                                        					{/section}	
								  						</select>
												...
												<div class="controls">
								  					<input  style="margin-left: -95px; width:80px; " id="idAut[]" disabled name="idAut[]" onblur="reviAuto(3);" type="text" value="">
												</div>
										...
										<tbody id="tbod">
                                			<tr>
                                				<td colspan="2">
													<label style="float: left; width:70; padding-top: 5px; text-align: right;" for="selectTipoPago">T. Pago</label>
													<div class="controls">
								  						<select onChange="revSeleccion(4)" disabled id="selectTipoPago[]" style="margin-left: -95px; width:110px; ">
                                        					{section name="p" loop=$LISTA_FPAGOS}
                                            					<option value="{$LISTA_FPAGOS[p].id}">{$LISTA_FPAGOS[p].descCorta}</option>
                                        					{/section}	
								  						</select>
												...
												<div class="controls">
								  					<input  style="margin-left: -95px; width:80px; " id="idAut[]" disabled name="idAut[]" onblur="reviAuto(4);" type="text" value="">
												</div>
							...
							<div class="form-actions" style="text-align:right;margin-top:-15px;">
								...
								<button type="button" disabled onclick="impFactu();" id="btnImp" class="btn btn-large btn-primary"><i class="icon-print icon-white"></i> Imprimir</button>
								 <!-- 
...
$(document).ready(function(){  
	...
	$("#fpagaCon2").change(function (e) {
		...
		var vPagoCon1 = parseFloat(res);
		var vPagoCon2 = parseFloat($('#fpagaCon2').val());
		var vTotal = parseFloat($('#ftotal').val());
		var vVuelto = vPagoCon1 + vPagoCon2 - vTotal;
		$('#fvuelto').val(vVuelto);
		if (vVuelto >= 0.00) {
        	document.getElementById('message-fields').innerHTML = "";
        	$("#btnImp").prop("disabled",false);
        	
        	var valFinal = Formatea($('#fpagaCon2').val());
...
$("#fpagaCon3").change(function (e) {
		...
		var res = str.replace(",", "");
		var vPagoCon2 = parseFloat(res);
		
		
		var vPagoCon3 = parseFloat($('#fpagaCon3').val());
		var vTotal = parseFloat($('#ftotal').val());
		var vVuelto = vPagoCon1 + vPagoCon2 + vPagoCon3 - vTotal;
		...
		if (vVuelto >= 0.00) {
        	...
        	var valFinal = Formatea($('#fpagaCon3').val());
...
$("#fpagaCon4").change(function (e) {
	...
	var res = str.replace(",", "");
	var vPagoCon3 = parseFloat(res);
	
	
	var vPagoCon4 = parseFloat($('#fpagaCon4').val());
	var vTotal = parseFloat($('#ftotal').val());
	var vVuelto = vPagoCon1 + vPagoCon2 + vPagoCon3 + vPagoCon4 - vTotal;
	...
	if (vVuelto >= 0.00) {
    	...
    	var valFinal = Formatea($('#fpagaCon4').val());
                        		...
                        		//    alert($dataStr[0]);
                        	    	...
                        	    	//	$("#btnImp").focus();
                        				...
                        				if ($dataStr[0]=='NO'){
                        					//nota no existe.
                        					$("#btnImp").prop("disabled",true);
                        	    			document.getElementById('message-fields').innerHTML = "<div class='alert alert-error center' style=''><strong><span class='icon icon-red icon-alert'/></span> Nota de Crédito no existe.</strong></div>";
                        	    			$idAut.focus(); 	
                        				}else{
                        					alert(data);
...
//	    }
      ...
      var properties = {
                        		...
                        		cache: false,
                        		datatype:"text",
                        		data : "idTienda="+$('#idTienda').val()+"&idFactura="+$('#idFactura').val()+"&formaPago1="+$("#selectTipoPago1").val()+"&autorizacion1="+$("#idAut1").val()+"&montoPago1="+vPagoCon1+"&formaPago2="+$("#selectTipoPago2").val()+"&autorizacion2="+$("#idAut2").val()+"&montoPago2="+vPagoCon2+"&formaPago3="+$("#selectTipoPago3").val()+"&autorizacion3="+$("#idAut3").val()+"&montoPago3="+vPagoCon3+"&formaPago4="+$("#selectTipoPago4").val()+"&autorizacion4="+$("#idAut4").val()+"&montoPago4="+vPagoCon4,	
                        		type : "GET",
                        		success: function(data){
                        			if (data.trim()!='OK') {
                        				alert(data);
                        				...
                        				window.location.href ='ImpreFacPro.php?idDoc='+$('#idFactura').val()+'&indTipoDoc='+'F'; // factura
                        				//$dummy="";
...
function revSeleccion(numPago){
	...
	// E: EFECTIVO, T: TARJETA, R: TRANSFERENCIA, D: DIFERIDA, C: CHEQUE ; N: NOTA
	if (vValor=="E"){
		  //paga en efectivo
		...
		if (vValor=="T" || vValor=="C" || vValor=="R" || vValor=="N") {
...
function getDetFactura(idFactura){
	...
	$("#selectTipoPago[1]").prop("disabled",false);
	$("#idAut[1]").prop("disabled",false);
	//$('#fpagaCon').val(detFactura[6]);
	
	var vValor = detFactura[6];
	var vValorP = vValor.split(".");
	var entPart = vValorP[0];
	var decPart = vValorP[1];
    ...
    $('#selectTipoPago[1]').prop('selectedIndex',0);
    ...
    $("#fpagaCon[1]").prop("disabled",false);
