...
class modelBill{
	...
	function GetFieldHide($userName,$fieldName) {
		...
		return $data;
         ...
         function inCliente($nombre, $telefono, $correo,$contacto,$vendedor,$codTienda){
            
            $sql = "call spInsClienteFacT('$nombre', '$telefono', '$correo','$contacto','$vendedor',$codTienda,@out_idCliente,@out_status);";
...
//echo $sql;exit;
        ...
        function insDetFactura($tienda,$idFactura, $numLinea, $codArti, $desArticulo,$cantidad, $precioVenta,$pdesc,$totPrice,$costo) {
            $sql = "call spInsDetFactura($tienda,$idFactura,$numLinea,'$codArti','$desArticulo',$cantidad,$precioVenta,$pdesc,$totPrice,$costo,@out_status);";
    ...
    function getAutDescGere(){
    	$sql = "call spGetAutDescGere(@out_status);";
    	$rs = $this->db->Execute ($sql);
    	$data = $rs->getArray();
    	$rs->Close();
    	return $data;
