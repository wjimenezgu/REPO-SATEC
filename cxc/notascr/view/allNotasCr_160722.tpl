					...
					<table class="table table-striped table-bordered bootstrap-datatable" id="tblNotas">
						...
						{section name=lP loop=$LISTA_NOTASCR}
							<tr style="padding:0px;" id="tr{$LISTA_NOTASCR[lP].idNota}">
								...
								<td class="center">
										<a href="#" id="{$LISTA_NOTASCR[lP].idNota}|{$LISTA_NOTASCR[lP].totalApli}|{$LISTA_NOTASCR[lP].saldoNota}"  onclick="goPage(6, this.id)" Title="Imprimir Nota"><i class="icon icon-color icon-print"></i></a>										
								</td>                                
							</tr> 
						{/section}
					</table>
...
function goPage(action, idNota){
	...
	if(action == 6){
		...
		if( action == 7){
			...
			detNota = idNota.split("|");
			window.location.href ='NotaCr.php?action='+action+"&id="+detNota[0];
		}
	}
}
