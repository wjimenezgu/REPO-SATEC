			...
			<tbody id="infoFact" >
				{section name=lP loop=$LISTA_MOVI}
					<tr style="padding:0px;"> 
						<td>{if $LISTA_MOVI[lP]["NumNota"] eq 'TOTAL'}  {else}{$LISTA_MOVI[lP]["NumNota"]} {/if}</td>  
						<td>{if $LISTA_MOVI[lP]["NumNota"] eq 'TOTAL'} TOTAL {else}{$LISTA_MOVI[lP]["FecNota"]} {/if}</td>
						...
						<td style="text-align:right;">{if $LISTA_MOVI[lP]["NumNota"] eq 'TOTAL'}  {else}{$LISTA_MOVI[lP]["PorceDesc"]} {/if}</td>
