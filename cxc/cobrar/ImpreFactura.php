<?php 
    ...
    foreach ($detalleDocu as $dato){
        ...
        if (strlen($dato["PrecioVenta"]) >= 12){
        	str_replace(',','',$dato["PrecioVenta"]);
        }
        $cmds .= substr('         ',1,10-strlen($dato["PrecioVenta"])).$dato["PrecioVenta"].' ';
        $cmds .= substr('   ',1,3-strlen($dato["PorceDesc"])).$dato["PorceDesc"].' ';
        if (strlen($dato["PrecioTotal"]) >= 12){
        	str_replace(',','',$dato["PrecioTotal"]);
        }
...
<body>
    ...
    <div style="width:100%">
    	...
    	<input type="button" id="imprime" autofocus name="imprime"   style="margin-left:25%; margin-top:25%;font-size:48px" onclick="javascript:jsWebClientPrint.print('cmds=' + $('#cmds').val()+'&idTienda=' + $('#idTienda').val());<?php if($idTienda != 0){?>  window.location.href ='../facturacion/index.php'; <?php }if ($idTienda == 0){?>window.location.href ='../cobrar/index.php' <?php } ?>" value="Presionar Enter para regresar ..." />
    ...
    echo WebClientPrint::createScript(Utils::getRoot().'/CRM-SATEC/sInventario/cobrar/PrintFactura.php')
    ?>
