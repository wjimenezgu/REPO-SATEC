{$NAV_MENU}
    ...
    // echo WebClientPrint::createScript(Utils::getRoot().'/PrintESCPOSReceiptSample/PrintReceipt.php')
...
function reviAuto(numPago){
	  ...
	  if (vValor=="T" || vValor=="C" || vValor=="N" || vValor=="R" || vValor=="O"){
    		if ($idAut.val()!=""){
    		document.getElementById('message-fields').innerHTML = "";
      ...
      var properties = {
                        		...
                        		success: function(data){
                        			if (data.trim()!='OK') {
                        				...
                        				if ($esManual == "N"){
                        					window.location.href ='ImpreFacProV2.php?idDoc='+$('#idFactura').val()+'&indTipoDoc='+'F'; // factura
...
function revSeleccion(numPago){
	...
	//	alert("Efectivo");
		...
		if (vValor=="T" || vValor=="C" || vValor=="R" || vValor=="N" || vValor=="B" || vValor=="O") {
			  	...
			  	$idAut.prop("disabled",false);
		  	  	...
		  	  	// SOLO si es con nota de credito o Recibo se debe digitar el monto del pago con
		  	  	if (vValor=="N" || vValor=="B" || vValor=="O"){
		  		  $pagaCon.val(""); // se limpia para que indique explictamente el monto
		  		  $idAut.val("");
