<?php
    ...
    $numDoc  = $_GET["idNumDoc"];
    $fecDoc  = $_GET["idFecDoc"];

    $date         = explode("/", $_GET["idFecDoc"]) ;
    $newfecDoc       = $date[2]."-".$date[1]."-".$date[0];        
    
//    $newfecDoc = strftime("%Y-%d-%m", strtotime($fecDoc));    
    ...
    $mvc->assign("NUM_DOC",$numDoc);
    $mvc->assign("FEC_DOC",$fecDoc);
    $mvc->assign("DOCU_PEDIDO",$docuPedido);
    $mvc->assign("PED_DOC_DESGLOSE",$pedDocDesglose);
 
    
    
 /*   
    echo "<pre>";
    print_r($pedDocDesglose);
    echo "<pre>";
    exit;
 */   
        
      
    $page = $mvc->fetch("frmEditDoc.tpl");
