						...
						<table class="table table-striped table-bordered bootstrap-datatable" id="tblEventos">
						  <thead>
							  <tr>
								...
								{section name=lP loop=$LISTA_EVENTOS} 								
								<tr style="padding:0px;" id="tr{$LISTA_EVENTOS[lP].idEvento}">
  
     								<td style="width:40px;">{$LISTA_EVENTOS[lP].idEvento}</td>  
                                                                <td style="width:385px;" align="left">{$LISTA_EVENTOS[lP].descripcion|lower|capitalize}</td>
									<td class="center">
										<a href="#" id="{$LISTA_EVENTOS[lP].idEvento}" onclick="goPage(2, this.id)"><i class="icon icon-color icon-edit"></i></a>
										<a href="#" id="{$LISTA_EVENTOS[lP].idEvento}" onclick="goPage(3, this.id)"><i class="icon icon-color icon-remove"></i></a>
									</td>
								</tr> 
								{/section}
					  </table>  
