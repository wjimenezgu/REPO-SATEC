<?php
   ...
   // hasta aqui codigo agregado
    ...
    $mvc->setModule("commitmenttype");
    
    $mvc->assign("HEADER_PAGE", $header);
    $mvc->assign("FOOTER_PAGE", $footer);

    $idTipoCompromiso= $_GET["id"];
    
    $infoOpcionPago = $model->buscarDetalleTipoCompromiso($idTipoCompromiso);
    
    // Se crear la variable PROVEEDOR en el $mvc para utilarla dentro del procedimiento
    $mvc->assign("ID_TIPOCOMPROMISO", $idTipoCompromiso);
    $mvc->assign("TIPOCOMPROMISO", $infoOpcionPago);    
    
    $page = $mvc->fetch("editTipoCompromiso.tpl");
